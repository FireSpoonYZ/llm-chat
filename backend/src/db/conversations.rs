...
pub async fn create_conversation(
    ...
    sqlx::query_as::<_, Conversation>(
        "INSERT INTO conversations (id, user_id, title, system_prompt_override, provider, model_name, deep_thinking, image_provider, image_model)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
mod tests {
    ...
    async fn test_create_conversation_with_system_prompt() {
        let (pool, user_id) = setup().await;
        let conv = create_conversation(&pool, &user_id, "Prompted Chat", Some("You are a pirate."), None, None, false, None, None).await.unwrap();
    ...
    async fn test_create_conversation_with_provider_and_model() {
        ...
        let conv = create_conversation(
            ...
            Some("openai"), Some("gpt-4o"), false, None, None,
        ).await.unwrap();
    ...
    async fn test_create_conversation_with_all_fields() {
        ...
        let conv = create_conversation(
            ...
            Some("My Google"), Some("gemini-3-pro-image-preview"),
        ).await.unwrap();
        ...
        assert_eq!(conv.image_model.as_deref(), Some("gemini-3-pro-image-preview"));
    }

    #[tokio::test]
    async fn test_list_conversations() {
        let (pool, user_id) = setup().await;
        create_conversation(&pool, &user_id, "Chat 1", None, None, None, false, None, None).await.unwrap();
        create_conversation(&pool, &user_id, "Chat 2", None, None, None, false, None, None).await.unwrap();
    ...
    async fn test_get_conversation() {
        let (pool, user_id) = setup().await;
        let conv = create_conversation(&pool, &user_id, "Findable Chat", None, None, None, false, None, None).await.unwrap();
    ...
    async fn test_update_conversation() {
        let (pool, user_id) = setup().await;
        let conv = create_conversation(&pool, &user_id, "Old Title", None, None, None, false, None, None).await.unwrap();
    ...
    async fn test_delete_conversation() {
        let (pool, user_id) = setup().await;
        let conv = create_conversation(&pool, &user_id, "To Delete", None, None, None, false, None, None).await.unwrap();
    ...
    async fn test_get_other_users_conversation_returns_none() {
        ...
        let conv = create_conversation(&pool, &user_id, "Private Chat", None, None, None, false, None, None).await.unwrap();
